...
export default function CharactersPage() {
  ...
  const filteredCharacters = characters.filter(char => {
    ...
    const matchesAlignment = selectedAlignment === "All" || char.alignment === selectedAlignment;
    return matchesSearch && matchesAlignment;
  });

  const alignmentOptions = ["All", "Hero", "Villain", "Neutral", "Anti-Hero"];
    ...
    <div className="container mx-auto px-4 md:px-6 py-12">
      <div className="mb-12 space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          <h1 className="font-orbitron text-4xl md:text-5xl font-bold">
            CHARACTER <span className="text-omnitrix-green">DOSSIER</span>
          </h1>
        ...
        <div className="flex flex-wrap items-center gap-4 border-b border-white/5 pb-6">
          <div className="flex items-center gap-2 text-white/40 mr-4">
            ...
            <span className="font-orbitron text-xs uppercase tracking-widest">Alignment:</span>
          </div>
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-full font-orbitron text-xs uppercase tracking-widest transition-all ${
                  selectedAlignment === align
                    ? align === "Hero" ? "bg-omnitrix-green text-black font-bold" :
                      align === "Villain" ? "bg-villain-red text-white font-bold" :
      ...
      {filteredCharacters.length > 0 ? (
        ...
        <div className="py-24 text-center">
          ...
          <h3 className="font-orbitron text-xl font-bold mb-2">Character not found</h3>
          <p className="text-white/40">Try adjusting your search or alignment filter.</p>
        </div>
      )}
    </div>
  );
